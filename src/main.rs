...
fn start_search(period: N) {
    ...
    for n in 1..=period {
        ...
        recurse_search(period, &masks);
    }
}

fn recurse_search(period: N, masks: &[u64]) {
    // search for the position with the fewest (but still > 1) possible numbers
    // and branch on that position
    let mut best_branch_pos = 0;
    let mut best_branch_heuristic = std::usize::MAX;
    let mut prev_branch_pos_num_choices = 1;
    for branch_pos in 1..period {
        let num_choices = masks[branch_pos].count_ones() as usize;
        if num_choices == 1 {
            prev_branch_pos_num_choices = num_choices;
            continue;
        }
        debug_assert_ne!(num_choices, 0);
        //let num_choices_left = masks[branch_pos-1].count_ones() as usize;
        //let num_choices_right = masks[(branch_pos+1) % period].count_ones() as usize;
        //let heuristic = num_choices * period * 4 + num_choices_left * 3 + num_choices_right;
        let heuristic = num_choices * period + prev_branch_pos_num_choices;
        if heuristic < best_branch_heuristic {
            best_branch_pos = branch_pos;
            best_branch_heuristic = heuristic;
        }
        prev_branch_pos_num_choices = num_choices;
    }

    if best_branch_pos == 0 {
        if is_trivial(masks) {
    ...
    for n in 1..=period {
        let n_mask = 1 << n;
        if masks[best_branch_pos] & n_mask == 0 {
            continue;
        }

        let mut masks = masks[0..period].to_owned();

        masks[best_branch_pos] = n_mask;
        ...
        // do some first iterations that are simple and fast, but likely to be effective
        debug_assert!(best_branch_pos >= 1);
        project_back(best_branch_pos-1, &mut loop_state);
        if loop_state.impossible {
            continue;
        }
        for j in best_branch_pos..period {
